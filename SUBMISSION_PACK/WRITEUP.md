...
two layers say. Measured over 4,000 synthetic readings drawn from a grid far
wider than any of these channels can physically reach, the AI multiplier
...
- **There is one computer, not twenty.** The design is decentralised — every
  node is scored on its own readings, there is no central authority, and
  agreement is asked of neighbours within 6 km rather than of a server — but
  all 20 nodes currently run as peers inside one process on one Pi 5. Putting
  the peers on separate devices is §10's third item, not something we are
...
- **228 automated tests pass** on Python 3.11, 3.12 and 3.13, including a test
...
- `python scripts/judge_validate.py` runs the smoke test, all 228 tests, a
...
things that matter, on 1,320 independently seeded frames. Each configuration
...
The AI column is honest about itself: on all three of these measures it scores
exactly what the thresholds alone score. Where it earns its place is in
between — at intensity 0.4 it detects 8.3 % of frames against the thresholds'
2.5 %, and at 0.6 it detects 75.0 % against 50.8 % — and in the explanation it
attaches to every alert. It is not what stops the false alarms; the mesh is.
...
is real but only half-formed, the mesh detects 35–62 % of frames against
...
a prebuilt Arm wheel with compilation forbidden, all 228 tests passed in
12–14 s across runs, the model trained in 0.1 s, a full 20-node cycle took
13–14 ms and the engine peaked at 199 MB.
...
brought that to 228. We worked on it together throughout, and we would both
...
fixed the bugs it found; added tests (the suite grew from 144 to 228); batched
...
python scripts/judge_validate.py        # smoke test + 228 tests + 2 demo cycles + export
